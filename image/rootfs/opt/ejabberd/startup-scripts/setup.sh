...
if [ ! -z $INSTALL_ADDITIONAL_NON_CORE_MODULE_1_NAME ] || [ ! -z $INSTALL_ADDITIONAL_NON_CORE_MODULE_2_NAME ] || [ ! -z $INSTALL_ADDITIONAL_NON_CORE_MODULE_3_NAME ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
# additionally installed modules from https://github.com/processone/ejabberd-contrib, defined by environment variables and mounted config files
EOF
if [ ! -z $INSTALL_ADDITIONAL_NON_CORE_MODULE_1_NAME ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $PATH_MODULE_CONFIG/$INSTALL_ADDITIONAL_NON_CORE_MODULE_1_NAME.yml
EOF
fi
if [ ! -z $INSTALL_ADDITIONAL_NON_CORE_MODULE_2_NAME ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $PATH_MODULE_CONFIG/$INSTALL_ADDITIONAL_NON_CORE_MODULE_2_NAME.yml
EOF
fi
if [ ! -z $INSTALL_ADDITIONAL_NON_CORE_MODULE_3_NAME ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $PATH_MODULE_CONFIG/$INSTALL_ADDITIONAL_NON_CORE_MODULE_3_NAME.yml
EOF
fi
