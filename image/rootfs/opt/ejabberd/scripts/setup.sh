...
CONFIGFILE=$HOME/conf/ejabberd.yml
echo ">>> ======================================================================="
...
if [ "${LISTENER_HTTP_ENABLED:-false}" = true ] \
   || [ "${LISTENER_HTTP_ACME_ENABLED:-true}" = true ]; then
  cat >> $CONFIGFILE <<EOF;
...
EOF
## Creating cert files wildcard reference if TLS is enabled somewhere
echo ">>> "
echo ">>> ======================================================================="
echo ">>> Setting TLS configuration parameters"
echo ">>> "
if [ "${LISTENER_C2S_STARTTLS:-false}" = true ] \
    || [ "${LISTENER_C2S_STARTTLS_REQUIRED:-false}" = true ] \
    || [ "${LISTENER_C2S_LEGACY_TLS_ENABLED:-false}" = true ] \
    || [ ! "${LISTENER_S2S_USE_STARTTLS:-false}" = false ] \
    || [ "${LISTENER_S2S_LEGACY_TLS_ENABLED:-false}" = true ] \
    || [ "${LISTENER_STUNSTURNS_TLS_ENABLED:-false}" = true ] \
    || [ "${LISTENER_HTTPS_ENABLED:-true}" = true ] \
    || [ "${LISTENER_MQTT_TLS:-false}" = true ]; then
  cat >> $CONFIGFILE <<EOF;
certfiles:
- $HOME/tls/*.pem
EOF
...
echo ">>> ======================================================================="
echo ">>> ###' Checking DH PARAM file existence at $HOME/conf/${DHPARAM_FILE_NAME:-dh.pem}"
echo ">>> "
if [ -f "$HOME/conf/${DHPARAM_FILE_NAME:-dh.pem}" ]; then
  echo ">>> dhparam file exists, using existing file"
  echo ">>> "
else
  echo ">>> generating dh param file with keysize ${DHPARAM_KEYSIZE:-2048}"
  echo ">>> "
  openssl dhparam -out $HOME/conf/${DHPARAM_FILE_NAME:-dh.pem} ${DHPARAM_KEYSIZE:-2048}
fi
cat >> $CONFIGFILE <<EOF;
...
cat >> $CONFIGFILE <<EOF;
    ...
    - "no_compression"
  'DHFILE': "$HOME/conf/${DHPARAM_FILE_NAME:-dh.pem}"
...
if [ "${LISTENER_S2S_LEGACY_TLS_ENABLED:-false}" = true ] \
     || [ "${LISTENER_S2S_ENABLED:-false}" = true ]; then
cat >> $CONFIGFILE <<EOF;
...
EOF
  ...
  if [ -z "${LDAP_BIND_PW:-}" ]; then
      if [ -z "${LDAP_BIND_PW_FILE:-}" ]; then
         echo ">>> No LDAP_BIND_PW or LDAP_BIND_PW_FILE is set"
      elif [ -n "${LDAP_BIND_PW_FILE:-}" ]; then
        # create secret from file
        LDAP_BIND_PW=$(cat "$LDAP_BIND_PW_FILE")
      fi
  fi
###
  cat > $HOME/conf/ldap.yml <<EOF;
...
EOF
if [ ! -z $LDAP_UID ] \
     || [ ! -z $LDAP_UID_ALTERNATIVE ] \
     || [ ! -z $LDAP_UID_ALTERNATIVE_2 ]; then
  cat >> $HOME/conf/ldap.yml <<EOF;
ldap_uids:
EOF
fi
...
ldap_tls_cacertfile: $HOME/tls/${LDAP_TLS_CA_CRT_FILE_NAME:-ca.pem}
ldap_tls_certfile: $HOME/tls/${LDAP_TLS_CRT_FILE_NAME:-server.pem}
EOF
fi
if [ ! -z $LDAP_SERVER_1 ] \
   || [ ! -z $LDAP_SERVER_2 ] \
   || [ ! -z $LDAP_SERVER_3 ]; then
  cat >> $HOME/conf/ldap.yml <<EOF;
# ldap hosts address, ports and backup server
ldap_servers: # list of ldap servers
EOF
elif [ -z $LDAP_SERVER_1 ] \
     || [ -z $LDAP_SERVER_2 ] \
     || [ -z $LDAP_SERVER_3 ]; then
  cat >> $HOME/conf/ldap.yml <<EOF;
...
if [ ! -z $LDAP_BACKUP_SERVER_1 ] \
   || [ ! -z $LDAP_BACKUP_SERVER_2 ] \
   || [ ! -z $LDAP_BACKUP_SERVER_3 ]; then
  cat >> $HOME/conf/ldap.yml <<EOF;
...
if [ "${LISTENER_C2S_LEGACY_TLS_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/c2s-tls.yml
EOF
cat > $HOME/conf/c2s-tls.yml <<EOF
listen:
  -
...
if [ "${LISTENER_S2S_LEGACY_TLS_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/s2s-tls.yml
EOF
cat > $HOME/conf/s2s-tls.yml <<EOF
listen:
  -
...
if [ "${LISTENER_STUNTURN_TCP_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/stun-turn-tcp.yml
EOF
cat > $HOME/conf/stun-turn-tcp.yml <<EOF
listen:
  -
...
if [ "${LISTENER_STUNTURN_UDP_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/stun-turn-udp.yml
EOF
cat > $HOME/conf/stun-turn-udp.yml <<EOF
listen:
  -
...
if [ "${LISTENER_STUNSTURNS_TLS_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/stuns-turns.yml
EOF
cat > $HOME/conf/stuns-turns.yml <<EOF
listen:
  -
...
# configure admin interface
  if [ "${LISTENER_HTTP_ADMIN_ENABLED:-false}" = true ]; then
...
EOF
  ...
  # configure ACME
  if [ "${LISTENER_HTTP_ACME_ENABLED:-true}" = true ]; then
  cat >> $HOME/conf/http.yml <<EOF
...
EOF
  ...
  # configure host-meta
  if [ "${LISTENER_HTTPS_HOST_META_ENABLED:-true}" = true ] \
        && ([ "${LISTENER_HTTPS_BOSH_ENABLED:-true}" = true ] \
        || [ "${LISTENER_HTTPS_WS_ENABLED:-true}" = true ]); then
  cat >> $HOME/conf/https.yml <<EOF
...
EOF
  ...
  # configure admin interface
  if [ "${LISTENER_HTTPS_ADMIN_ENABLED:-true}" = true ]; then
  cat >> $HOME/conf/https.yml <<EOF
      "/admin": ejabberd_web_admin
EOF
  fi
...
if [ ! "${DEFAULT_DB:-mnesia}" = mnesia ] || [ ! -z $DB_TYPE ] || [ "$DEFAULT_RAM_DB" = sql ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
include_config_file:
  - $HOME/conf/database.yml
EOF
  # check password existence
  if [ -z "${DB_PASSWORD:-}" ]; then
      if [ -z "${DB_PASSWORD_FILE:-}" ]; then
        echo ">>> No DB_PASSWORD or DB_PASSWORD_FILE are set"
  ...
  ###
cat > $HOME/conf/database.yml <<EOF;
# generic variables
...
EOF
    if [ "$DB_SSL_VERIFY" = true ]; then
      cat >> $HOME/conf/database.yml <<EOF;
sql_ssl_cafile: $HOME/tls/${DB_SSL_CAFILE_NAME:-ca.pem}
sql_ssl_certfile: $HOME/tls/${DB_SSL_CERTFILE_NAME:-server.pem}
EOF
    fi
  fi
fi
...
if [ "$DEFAULT_RAM_DB" = redis ] || [ "$REDIS_ENABLED" = true ]; then
  ...
  if [ -z "${REDIS_PASSWORD:-}" ]; then
      if [ -z "${REDIS_PASSWORD_FILE:-}" ]; then
        echo ">>> No REDIS_PASSWORD or REDIS_PASSWORD_FILE is set"
  ...
  - $HOME/conf/redis.yml
EOF
cat > $HOME/conf/redis.yml <<EOF;
redis_server: ${REDIS_SERVER_ADDRESS:-localhost}
redis_port: ${REDIS_SERVER_PORT:-6379}
redis_password: ${REDIS_PASSWORD:-} # default is empty string
...
acl:
  ...
  admin:
    user:
      - "${JID_ADMIN_USER0:-admin@${XMPP_DOMAIN1:-localhost}}"
...
if [ "${MOD_ADHOC_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_adhoc.yml
EOF
  if [ ! -f $HOME/conf/mod_adhoc.yml ]; then
    cat > $HOME/conf/mod_adhoc.yml <<EOF;
modules:
  mod_adhoc: {}
EOF
  fi
...
if [ "${MOD_ADMIN_EXTRA_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_admin_extra.yml
EOF
  if [ ! -f $HOME/conf/mod_admin_extra.yml ]; then
    cat > $HOME/conf/mod_admin_extra.yml <<EOF;
modules:
  mod_admin_extra: {}
EOF
  fi
...
if [ "${MOD_ANNOUNCE_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_announce.yml
EOF
  if [ ! -f $HOME/conf/mod_announce.yml ]; then
    cat > $HOME/conf/mod_announce.yml <<EOF;
modules:
  mod_announce:
    access: announce
EOF
...
### MOD_AVATAR #############################################
#
if [ "${MOD_AVATAR_ENABLED:-true}" = true ] \
     && [ "${MOD_PUBSUB_ENABLED:-true}" = true ] \
     && [ "${MOD_VCARD_ENABLED:-true}" = true ] \
     && [ "${MOD_VCARD_XUPDATE_ENABLED:-true}" = true ]; then
  echo ">>> creating link to ../mod_avatar.yml"
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_avatar.yml
EOF
  if [ ! -f $HOME/conf/mod_avatar.yml ]; then
    cat > $HOME/conf/mod_avatar.yml <<EOF;
modules:
  mod_avatar: {}
EOF
  fi
...
if [ "${MOD_BLOCKING_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_blocking.yml
EOF
  if [ ! -f $HOME/conf/mod_blocking.yml ]; then
    cat > $HOME/conf/mod_blocking.yml <<EOF;
modules:
  mod_blocking: {}
EOF
  fi
...
if [ "${LISTENER_HTTPS_BOSH_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_bosh.yml
EOF
  if [ ! -f $HOME/conf/mod_bosh.yml ]; then
    cat > $HOME/conf/mod_bosh.yml <<EOF;
modules:
  mod_bosh: {}
EOF
  fi
...
if [ "${MOD_CAPS_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_caps.yml
EOF
  if [ ! -f $HOME/conf/mod_caps.yml ]; then
    cat > $HOME/conf/mod_caps.yml <<EOF;
modules:
  mod_caps: {}
EOF
  fi
...
if [ "${MOD_CARBONCOPY_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_carboncopy.yml
EOF
  if [ ! -f $HOME/conf/mod_carboncopy.yml ]; then
    cat > $HOME/conf/mod_carboncopy.yml <<EOF;
modules:
  mod_carboncopy: {}
EOF
  fi
...
if [ "${MOD_CLIENT_STATE_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_client_state.yml
EOF
  if [ ! -f $HOME/conf/mod_client_state.yml ]; then
    cat > $HOME/conf/mod_client_state.yml <<EOF;
modules:
  mod_client_state: {}
EOF
  fi
...
if [ "${MOD_CONFIGURE_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_configure.yml
EOF
  if [ ! -f $HOME/conf/mod_configure.yml ]; then
    cat > $HOME/conf/mod_configure.yml <<EOF;
modules:
  mod_configure: {}
EOF
  fi
...
### MOD_CONVERSEJS #############################################
#
if [ "${LISTENER_HTTPS_CONVERSEJS_ENABLED:-false}" = true ] \
   && ([ "${LISTENER_HTTPS_BOSH_ENABLED:-true}" = true ] \
   || [ "${LISTENER_HTTPS_WS_ENABLED:-true}" = true ]); then
  echo ">>> creating link to ../mod_conversejs.yml"
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_conversejs.yml
EOF
  if [ ! -f $HOME/conf/mod_conversejs.yml ]; then
    cat > $HOME/conf/mod_conversejs.yml <<EOF;
modules:
  mod_conversejs:
EOF
  if [ "${LISTENER_HTTPS_WS_ENABLED:-true}" = true ]; then
    cat >> $HOME/conf/mod_conversejs.yml <<EOF;
    websocket_url: "wss://@HOST@:${LISTENER_HTTPS_PORT:-5443}/ws"
EOF
  else
    cat >> $HOME/conf/mod_conversejs.yml <<EOF;
    bosh_service_url: "https://@HOST@:${LISTENER_HTTPS_PORT:-5443}/bosh"
EOF
  fi
  fi
fi
...
if [ "${MOD_DISCO_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_disco.yml
EOF
  if [ ! -f $HOME/conf/mod_disco.yml ]; then
    cat > $HOME/conf/mod_disco.yml <<EOF;
modules:
  mod_disco: {}
EOF
  fi
...
if [ "${MOD_FAIL2BAN_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_fail2ban.yml
EOF
  if [ ! -f $HOME/conf/mod_fail2ban.yml ]; then
    cat > $HOME/conf/mod_fail2ban.yml <<EOF;
modules:
  mod_fail2ban: {}
EOF
  fi
...
if [ "${LISTENER_HTTPS_HOST_META_ENABLED:-true}" = true ] \
   && ([ "${LISTENER_HTTPS_BOSH_ENABLED:-true}" = true ] \
   || [ "${LISTENER_HTTPS_WS_ENABLED:-true}" = true ]); then
  echo ">>> creating link to ../mod_host_meta.yml"
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_host_meta.yml
EOF
  if [ ! -f $HOME/conf/mod_host_meta.yml ]; then
    cat > $HOME/conf/mod_host_meta.yml <<EOF;
modules:
  mod_host_meta:
EOF
    if [ "${LISTENER_HTTPS_BOSH_ENABLED:-true}" = true ]; then
...
if [ "${LISTENER_HTTPS_API_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_http_api.yml
EOF
  if [ ! -f $HOME/conf/mod_http_api.yml ]; then
    cat > $HOME/conf/mod_http_api.yml <<EOF;
modules:
  mod_http_api: {}
EOF
  fi
...
if [ "${LISTENER_HTTPS_FILESERVER_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_http_fileserver.yml
EOF
  if [ ! -f $HOME/conf/mod_http_fileserver.yml ]; then
    touch $HOME/logs/access.log
    cat > $HOME/conf/mod_http_fileserver.yml <<EOF;
modules:
  mod_http_fileserver:
    accesslog: $HOME/logs/access.log # default no access log
    docroot: $HOME/files
EOF
  fi
fi
...
if [ "${LISTENER_HTTPS_UPLOAD_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_http_upload.yml
EOF
  if [ ! -f $HOME/conf/mod_http_upload.yml ]; then
    cat > $HOME/conf/mod_http_upload.yml <<EOF;
modules:
  mod_http_upload:
...
if [ "${MOD_LAST_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_last.yml
EOF
  if [ ! -f $HOME/conf/mod_last.yml ]; then
    cat > $HOME/conf/mod_last.yml <<EOF;
modules:
  mod_last: {}
EOF
  fi
...
if [ "${MOD_MAM_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_mam.yml
EOF
  if [ ! -f $HOME/conf/mod_mam.yml ]; then
    cat > $HOME/conf/mod_mam.yml <<EOF;
modules:
  mod_mam:
...
if [ "${LISTENER_MQTT_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_mqtt.yml
EOF
  if [ ! -f $HOME/conf/mod_mqtt.yml ]; then
    cat > $HOME/conf/mod_mqtt.yml <<EOF;
modules:
  mod_mqtt: {}
EOF
  fi
...
if [ "${MOD_MUC_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_muc.yml
EOF
  if [ ! -f $HOME/conf/mod_muc.yml ]; then
    cat > $HOME/conf/mod_muc.yml <<EOF;
modules:
  mod_muc:
    access:
      - allow
...
if [ "${MOD_MUC_ADMIN_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_muc_admin.yml
EOF
  if [ ! -f $HOME/conf/mod_muc_admin.yml ]; then
    cat > $HOME/conf/mod_muc_admin.yml <<EOF;
modules:
  mod_muc_admin: {}
EOF
  fi
...
if [ "${MOD_OFFLINE_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_offline.yml
EOF
  if [ ! -f $HOME/conf/mod_offline.yml ]; then
    cat > $HOME/conf/mod_offline.yml <<EOF;
modules:
  mod_offline:
...
if [ "${MOD_PING_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_ping.yml
EOF
  if [ ! -f $HOME/conf/mod_ping.yml ]; then
    cat > $HOME/conf/mod_ping.yml <<EOF;
modules:
  mod_ping: {}
EOF
  fi
...
if [ "${MOD_PRIVACY_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_privacy.yml
EOF
  if [ ! -f $HOME/conf/mod_privacy.yml ]; then
    cat > $HOME/conf/mod_privacy.yml <<EOF;
modules:
  mod_privacy: {}
EOF
  fi
...
if [ "${MOD_PRIVATE_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_private.yml
EOF
  if [ ! -f $HOME/conf/mod_private.yml ]; then
    cat > $HOME/conf/mod_private.yml <<EOF;
modules:
  mod_private: {}
EOF
  fi
...
if [ "${MOD_PROXY65_ENABLED:-false}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_proxy65.yml
EOF
  if [ ! -f $HOME/conf/mod_proxy65.yml ]; then
    cat > $HOME/conf/mod_proxy65.yml <<EOF;
modules:
  mod_proxy65:
    access: local
    max_connections: 5
...
if [ "${MOD_PUBSUB_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_pubsub.yml
EOF
  if [ ! -f $HOME/conf/mod_pubsub.yml ]; then
    cat > $HOME/conf/mod_pubsub.yml <<EOF;
modules:
  mod_pubsub:
...
if [ "${MOD_PUSH_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_push.yml
EOF
  if [ ! -f $HOME/conf/mod_push.yml ]; then
    cat > $HOME/conf/mod_push.yml <<EOF;
modules:
  mod_push: {}
EOF
  fi
...
if [ "${MOD_PUSH_KEEPALIVE_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_push_keepalive.yml
EOF
  if [ ! -f $HOME/conf/mod_push_keepalive.yml ]; then
    cat > $HOME/conf/mod_push_keepalive.yml <<EOF;
modules:
  mod_push_keepalive: {}
EOF
  fi
...
if [ "${MOD_REGISTER_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_register.yml
EOF
  if [ ! -f $HOME/conf/mod_register.yml ]; then
...
if [ "${MOD_ROSTER_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_roster.yml
EOF
  if [ ! -f $HOME/conf/mod_roster.yml ]; then
    cat > $HOME/conf/mod_roster.yml <<EOF;
modules:
  mod_roster:
    versioning: true
EOF
...
if [ "${MOD_S2S_DIALBACK_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_s2s_dialback.yml
EOF
  if [ ! -f $HOME/conf/mod_s2s_dialback.yml ]; then
    cat > $HOME/conf/mod_s2s_dialback.yml <<EOF;
modules:
  mod_s2s_dialback: {}
EOF
  fi
...
if [ "${MOD_SHARED_ROSTER_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_shared_roster.yml
EOF
  if [ ! -f $HOME/conf/mod_shared_roster.yml ]; then
    cat > $HOME/conf/mod_shared_roster.yml <<EOF;
modules:
  mod_shared_roster: {}
EOF
  fi
...
if [ "${MOD_STREAM_MGMT_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_stream_mgmt.yml
EOF
  if [ ! -f $HOME/conf/mod_stream_mgmt.yml ]; then
    cat > $HOME/conf/mod_stream_mgmt.yml <<EOF;
modules:
  mod_stream_mgmt:
...
if [ "${MOD_STUN_DISCO_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_stun_disco.yml
EOF
  if [ ! -f $HOME/conf/mod_stun_disco.yml ]; then
    cat > $HOME/conf/mod_stun_disco.yml <<EOF;
modules:
  mod_stun_disco: {}
EOF
  fi
...
if [ "${MOD_VCARD_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_vcard.yml
EOF
  if [ ! -f $HOME/conf/mod_vcard.yml ]; then
    cat > $HOME/conf/mod_vcard.yml <<EOF;
modules:
  mod_vcard: {}
EOF
  fi
...
if [ "${MOD_VCARD_XUPDATE_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_vcard_xupdate.yml
EOF
  if [ ! -f $HOME/conf/mod_vcard_xupdate.yml ]; then
    cat > $HOME/conf/mod_vcard_xupdate.yml <<EOF;
modules:
  mod_vcard_xupdate: {}
EOF
  fi
...
if [ "${MOD_VERSION_ENABLED:-true}" = true ]; then
  ...
  cat >> $CONFIGFILE <<EOF;
  - $HOME/conf/mod_version.yml
EOF
  if [ ! -f $HOME/conf/mod_version.yml ]; then
    cat > $HOME/conf/mod_version.yml <<EOF;
modules:
  mod_version:
    show_os: false
EOF
...
#
if [ ! -z $ADDITIONAL_CORE_MODULE_1_NAME ] \
   || [ ! -z $ADDITIONAL_CORE_MODULE_2_NAME ] \
   || [ ! -z $ADDITIONAL_CORE_MODULE_3_NAME ]; then
  echo ">>> "
  echo ">>> ======================================================================="
...
#
if [ ! -z $INSTALL_ADDITIONAL_NON_CORE_MODULE_1_NAME ] \
